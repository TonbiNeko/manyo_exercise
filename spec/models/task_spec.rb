  ...
  describe '検索機能' do
    ...
    context 'scopeメソッドでタイトルのあいまい検索とステータス検索をした場合' do
      it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
        expect_task = Task.search_with_name('task').search_with_status(2)
        expect(expect_task.count).to eq 1
        expect_task = Task.search_with_name('sample').search_with_status(2)
        expect(expect_task.count).to eq 1
        expect_task = Task.search_with_name('sample').search_with_status(1)
